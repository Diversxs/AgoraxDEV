...
<div class=" rounded overflow-hidden shadow-lg flex flex-col items-center m-5">
    ...
    <div class="px-6 py-4">
        ...
        <div class="flex-auto flex space-x-3 m-5 justify-center">
            ...
            <form action="{{ route('admin_destroy',$event->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <a class="w-1/2 flex items-center justify-center rounded-md bg-black text-white" href="{{route('admin_destroy',$event->id)}}"><button  type="submit">Delete</button></a>
            </form>
        </div>
    </div>
