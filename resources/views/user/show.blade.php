...
<div class="btn_home text-center md:text-left flex justify-center">
    ...
    <div v-for="card in cards"
        ...
        <div class="w-full px-6 text-gray-50 flex flex-col justify-between ">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center sm:items-around ">
                ...
                @if($event->isSubcribed(Auth::user()) === false)
                    <a class="bg-red-700 hover:bg-red-700 text-white text-sm px-4 py-2   rounded-full" method="GET" href="{{ route('subscribe', $event->id)}}"><button  type="submit">book</button></a>
                    @endif
                    @if($event->isSubcribed(Auth::user()) === true)
                    <a class="bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2   rounded-full" method="GET" href="{{ route('unSubscribe', $event->id)}}"><button  type="submit">Cancel booking</button></a>
                    @endif
            </div>
        </div>
    </div>
