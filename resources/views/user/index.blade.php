      ...
      <div class="buttons flex justify-between">
          <a href="{{ route('passedEvents') }}">
              <button class="ml-8 my-8 bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2  border rounded-full"
                  type="submit">Check our past events</button>
          </a>
          <a class="mr-10 text-center md:text-right" method="Post " href="{{ route('userEvents') }}"><button
                  class="ml-8 my-8 bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2  border rounded-full"
                          ...
                          <div class="mr-10 ml-10 mt-0">
                              ...
                              <div class="mb-5 flex mt-5 justify-between items-center flex-col md:flex-row ">
                                  ...
                                  <div class="mb-5 flex inline-flex items-center ml-10">
                                      ...
                                      <p class="text-md text-blue-900 ml-3">
                                          {{ $event->capacity - count($event->bookedInUsers) }}
                                      </p>
                                  </div>
                                  <div class="mb-5 flex items-center">
                                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="red"
                                          viewBox="0 0 24 24" stroke="currentColor">
                                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  ...
                                  @if ($event->isSubcribed($user) === false)
                                      <a class="bg-green-600 hover:bg-red-700 text-white text-sm px-4 py-2  border rounded-full"
                                          method="GET" href="{{ route('subscribe', $event->id) }}"><button
                                              type="submit">Book this Event</button></a>
                                  @endif
                                  @if ($event->isSubcribed($user) === true)
                                      <a class="bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2  border rounded-full"
                                          method="GET" href="{{ route('unSubscribe', $event->id) }}"><button
                                              type="submit">Cancel booking</button></a>
                                  @endif
                              </div>
                          </div>
                      </section>
