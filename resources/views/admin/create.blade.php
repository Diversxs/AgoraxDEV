    ...
    <form action="{{ route('admin_store') }}" method="POST">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
        ...
        <div class="grid grid-cols-1">
          ...
          <input class="py-2 px-3 rounded-lg border-2 text-gray-500 border-red-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="date" placeholder="Date" name="date" />
        </div>
      ...
      <div class="grid grid-cols-1 mt-5 mx-7">
            ...
            <input type="checkbox" name="is_featured" class="switch-input" value=true {{ old('is_featured') ? 'checked="checked"' : '' }}/>
          </div>
      <div class="grid grid-cols-1 mt-5 mx-7">
        ...
        <div class='flex items-center justify-center w-full'>
          <label class='flex flex-col border-4 border-dashed w-full h-32 hover:bg-white-100 hover:border-red-300 group'>
            <div class='flex flex-col items-center justify-center pt-7'>
              <svg class="w-10 h-10 text-gray-400 group-hover:text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
